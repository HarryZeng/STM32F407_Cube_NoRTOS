...
int32_t hSpeed_Buffer2[SPEED_BUFFER_SIZE]={0}, hSpeed_Buffer1[SPEED_BUFFER_SIZE]={0};//左右轮速度缓存数组
...
static volatile uint16_t hEncoder_Timer_Overflow1;//电机B编码数采集 
static volatile uint16_t hEncoder_Timer_Overflow2;//电机A编码数采集
...
int16_t ENC_Calc_Rot_Speed2(void)//计算电机A的编码数
{   
    int32_t wDelta_angle;
    uint16_t hEnc_Timer_Overflow_sample_one;
    uint16_t hCurrent_angle_sample_one;
    int32_t temp;
    int16_t haux;
...
int16_t ENC_Calc_Rot_Speed1(void)//计算电机B的编码数
{   
    int32_t wDelta_angle;
    uint16_t hEnc_Timer_Overflow_sample_one;
    uint16_t hCurrent_angle_sample_one;
...
void Gain1(void)//设置电机B PID调节 PA1
{
	...
	TIM13->CCR1 = pulse1;//电机B赋值PWM
	//TIM2->CCR3 = A_REMP_PLUS;//电机A赋值PWM
    TIM14->CCR1 = pulse;//电机A赋值PWM
}
